...
app.get('/api/debug/paths', (req, res) => {
    ...
    let publicHtmlContents: string[] = [];
    try { publicHtmlContents = fs.readdirSync(publicHtml).slice(0, 40); } catch(e) { publicHtmlContents = ['ERROR: ' + String(e)]; }
    
    let nodejsContents: string[] = [];
    try { nodejsContents = fs.readdirSync(cwd).slice(0, 40); } catch(e) { nodejsContents = ['ERROR: ' + String(e)]; }
