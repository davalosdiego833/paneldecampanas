...
const populateDirectoryFromSnapshot = () => {
    ...
    if (safeExists(snapPath)) {
        try {
            ...
            if (data.campaigns) {
                Object.values(data.campaigns).forEach((arr: any) => {
...
function servirPremiosCarpeta(carpeta: string) {
    return (req: express.Request, res: express.Response) => {
        try {
            ...
            const filePath = candidatePaths.find(p => safeExists(p));
            if (!filePath) return res.status(404).json({ error: 'Reporte aún no descargado' });
            const data = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
            res.json(data);
        } catch (error) {
            console.error('Error leyendo premios:', error);
...
app.get('/api/resumen-general', (req, res) => {
    try {
        ...
        let selectedDates: Record<string, any> = {};
        if (dates) {
            try { selectedDates = JSON.parse(dates as string); } catch (e) {}
        }
...
app.post('/api/push/unsubscribe', (req, res) => {
    try {
        ...
        res.json({ success: true });
    } catch (e) { res.status(500).json({ error: 'Unsubscribe error' }); }
});

const sendOneSignalDirect = async ({ group, title, body, url }: { group: string; title: string; body: string; url: string }) => {
    const candidateFiles = [
    ...
    try {
        ...
        if (!cfg || !cfg.appId || !cfg.apiKey || !cfg.enabled) return null;
        ...
        for (const tp of targetPaths) {
            try {
                if (fs.existsSync(tp)) {
                    ...
                    if (Array.isArray(subs)) {
                        subs = subs.filter((s: any) => s.subscription?.endpoint !== endpoint && s.endpoint !== endpoint);
                        fs.writeFileSync(tp, JSON.stringify(subs, null, 2));
                    }
                }
            } catch (e) {}
