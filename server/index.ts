...
app.get('/api/daniela/datos', (req, res) => {
    try {
        ...
        const pagado_pendiente_por_asesor = pag.map((r: any) => ({
        ...
        })).filter((r: any) => r.pagado.polizas > 0 || r.pendiente.polizas > 0);

        // 3. Proactivos
        const proactivos_list = rg.proactivos || [];
        const proactivos_activos = proactivos_list
            .filter((p: any) => String(p.Proactivo_al_mes).trim().toUpperCase() === 'SÍ')
            .map((p: any) => ({
                asesor: p.ASESOR,
                sucursal: p.SUC,
        ...
        if (originalCampaigns.mdrt) {
            campaigns.mdrt = originalCampaigns.mdrt
                .filter((c: any) => Number(c.PA_Acumulada) > 0)
        ...
        if (originalCampaigns.convenciones) {
            campaigns.convenciones = originalCampaigns.convenciones
                .filter((c: any) => Number(c.PA_Total) > 0 || Number(c.Polizas) > 0)
                .map((c: any) => ({
                    asesor: c.Asesor,
                    clave: c.Clave,
                    pa_total: c.PA_Total,
        ...
        if (originalCampaigns.legion_centurion) {
            campaigns.legion_centurion = originalCampaigns.legion_centurion
                .filter((c: any) => Number(c.Total_Polizas) > 0)
                .map((c: any) => ({
                    asesor: c.Asesor,
                    clave: c.Clave,
        ...
        if (originalCampaigns.camino_cumbre) {
            campaigns.camino_cumbre = originalCampaigns.camino_cumbre
                .filter((c: any) => Number(c.Polizas_Totales) > 0)
                .map((c: any) => ({
                    asesor: c.Asesor,
                    clave: c.Clave,
        ...
        if (originalCampaigns.fanfest) {
            campaigns.fanfest = originalCampaigns.fanfest
                .filter((c: any) => Number(c.Total_Polizas) > 0 || String(c.Premio).trim() === 'GANADO')
                .map((c: any) => ({
                    asesor: c.Asesor,
                    clave: c.Clave,
                    total_polizas: c.Total_Polizas,
        ...
        if (originalCampaigns.vive_tu_pasion) {
            campaigns.vive_tu_pasion = originalCampaigns.vive_tu_pasion
                .filter((c: any) => Number(c.Polizas) > 0 || String(c.Premio).trim() !== '')
                .map((c: any) => ({
                    asesor: c.Asesor,
                    clave: c.Clave,
                    polizas: c.Polizas,
        ...
        if (originalCampaigns.graduacion) {
            campaigns.graduacion = originalCampaigns.graduacion.map((c: any) => ({
                asesor: c.Asesor,
                clave: c.Clave,
                mes_asesor: c.Mes_Asesor,
                polizas_totales: c.Polizas_Totales,
