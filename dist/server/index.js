...
const readExcelData = (folderName) => {
    ...
    try {
        ...
        if (json.length > 0)
            console.log('[DEBUG] First row keys:', Object.keys(json[0]));
        return json;
    }
    catch (error) {
...
app.get('/api/campaign/:name/data/:advisor', (req, res) => {
    ...
    const data = readExcelData(name);
    if (!data)
        return res.status(404).json({ error: 'Campaign not found' });
    const advisorData = data.find((row) => row.Asesor === advisor);
    if (!advisorData)
        return res.status(404).json({ error: 'Advisor not found in this campaign' });
