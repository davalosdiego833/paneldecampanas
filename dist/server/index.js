...
const getProtectedPath = (folder) => {
    ...
    const candidates = [
        `/home/u211138134/domains/panel.ambrizydavalos.com/${folder}`,
        ...
        path.join(safeDirname, folder),
        path.join(safeDirname, 'dist', folder)
    ];
...
const extractCutoffDate = (wb, type) => {
    try {
        ...
        if (type === 'mdrt') {
            ...
            let sheetsToScan = sheetNames.filter((n) => wb.SheetNames.includes(n)).map((n) => wb.Sheets[n]);
        ...
        try {
            ...
            for (const dirent of dirents) {
                ...
                if (dirent.name.toLowerCase().includes('julio'))
                    continue; // filter out old July bases
...
app.get('/api/ventas-mensuales/latest', (req, res) => {
    try {
        ...
        const filePath = candidatePaths.find(p => safeExists(p));
        if (!filePath)
            return res.status(404).json({ error: 'Aún no se ha publicado el cierre de mes' });
        const data = JSON.parse(fs.readFileSync(filePath, 'utf-8'));
        res.json(data);
    }
    catch (error) {
...
app.get('/api/campaign/:name/data/:advisor', (req, res) => {
        ...
        try {
            ...
            if (campaigns) {
                ...
                if (campaigns[campaignKey]) {
                    const campData = campaigns[campaignKey];
                    const dir = getAdvisorDirectory();
                    ...
                    if (row) {
                        ...
                        if (normName === 'mdrt') {
                            ...
                            if (fechaCorte) {
                                ...
                                if (mMatch)
                                    currentMonthNum = MONTHS_ES.indexOf(mMatch[1]) + 1;
                            }
                            return res.json({
                                Asesor: row.Asesor, Clave: row.Clave, Fecha_Corte: fechaCorte,
    ...
    if (normName === 'mdrt') {
        ...
        if (cutoffDate) {
            ...
            if (parts.length === 3)
                currentMonthNum = parseInt(parts[1], 10);
        }
        return res.json({
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                console.error('[SNAPSHOT] Error reading asesores_sin_emision:', e);
            }
            // --- Pagado y Emitido ---
            try {
                ...
                console.error('[SNAPSHOT] Error reading pagado_emitidido:', e);
            }
            // --- Proactivos ---
            try {
                ...
                console.error('[SNAPSHOT] Error reading proactivos:', e);
            }
            // --- Comparativo de Vida ---
            try {
                ...
                if (wbComp) {
                    ...
                    if (wsA) {
                        ...
                        const sumField = (key) => inds.reduce((s, r) => s + (Number(r[key]) || 0), 0);
                ...
                console.error('[SNAPSHOT] Error reading comparativo_vida:', e);
            }
        ...
        try {
            const now = new Date();
            const fmtMx = (opts) => new Intl.DateTimeFormat('es-MX', { ...opts, timeZone: 'America/Mexico_City' }).format(now);
...
app.post('/api/estatus-polizas/comentarios', (req, res) => {
    try {
        ...
        if (!poliza)
            return res.status(400).json({ error: 'poliza is required' });
        const now = new Date();
        ...
        console.error('Error saving comentario:', e);
        res.status(500).json({ error: 'Write error' });
    }
});
// ===================== ACTIVIDAD DEL STAFF =====================
...
// Diagnostic: log what Hostinger gives us
console.log('[BOOT] process.env.PORT =', process.env.PORT);
console.log('[BOOT] process.env.NODE_ENV =', process.env.NODE_ENV);
console.log('[BOOT] process.env.HOST =', process.env.HOST);
console.log('[BOOT] CWD =', process.cwd());
console.log('[BOOT] safeDirname =', safeDirname);
