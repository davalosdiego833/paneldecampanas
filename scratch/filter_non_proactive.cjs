...
const data = XLSX.utils.sheet_to_json(ws, { range: 5 }); // Row 5 is header
...
const nonProactiveOlderAdvisors = [];

data.forEach((r, idx) => {
    const matVal = String(r['Mat'] || r['Matriz'] || '').trim();
    if (matVal !== '2043') return; // Only Matriz 2043
    ...
    const isProactive = proactivoVal === 'p';

    if (!isProactive) {
        nonProactiveOlderAdvisors.push({
            name,
            clave,
            connectionDate: conDate.toLocaleDateString('es-MX', { day: 'numeric', month: 'long', year: 'numeric' }),
            months,
            conVal,
            row: idx + 7 // Actual row in excel sheet (0-indexed + header rows)
        });
    }
});

console.log('--- 13 MONTHS OR MORE NON-PROACTIVE ADVISORS ---');
const filtered = nonProactiveOlderAdvisors.filter(a => a.months >= 13);
filtered.forEach(a => {
    console.log(`Asesor: ${a.name} (${a.clave}) | Conexión: ${a.connectionDate} | Meses: ${a.months}`);
});
